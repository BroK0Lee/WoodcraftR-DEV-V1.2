...
import { useInitializePanel } from "@/hooks/useInitializePanel";   // ← nouveau
...
function Model({ url, dimensions }: Pick<Props, "url" | "dimensions">) {
  // Pré-chargement du GLB pour éviter un flash
  if (useGLTF.preload) {
...
export default function GLBViewer({ url, target, dimensions, edges }: Props) {
  /** Injecte automatiquement la première op *resize* avec les dimensions par défaut */
  useInitializePanel();
